											...
											<div class="row">
												...
												<div class="col-lg-3 mb-4">
													<div for="" class="parent-vertical-center" permission="{{$permission->name}}">
														<div class="custom-control custom-switch">
															<input class="custom-control-input" id="{{$permission->id}}" type="checkbox" name="permissions[]" value="{{$permission->name}}" @foreach($permissionsAssigned as $per) {{ $per === $permission->name ? 'checked' : '' }} @endforeach {{ $permission->name == 'admin.dashboard' || $permission->name == 'admin.profile' || $permission->name == 'admin.profile.update' ? 'checked disabled' : '' }}>
